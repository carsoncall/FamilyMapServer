            ...
            if (exchange.getRequestMethod().toLowerCase(Locale.ROOT).equals("get")) {
                ...
                if (requestHeaders.containsKey("Authorization")) {
                    ...
                    if (URISplit.length >= 3) {
                        personID = URISplit[2];
                        request = new PersonRequest(personID,authtoken);
                    } else {
                        request = new PersonRequest(null, authtoken);
                    }

                    PersonService personService = new PersonService();
                    personResult = personService.retrieve(request);
                    ...
                    try (Writer w = new OutputStreamWriter(resBody, StandardCharsets.UTF_8)) {
                        w.write(new Gson().toJson(personResult));
                    }
